...
| 항목      | 내용                                             |
|---------|------------------------------------------------|
| **목표**  | 서버도 평문을 볼 수 없는 E2E 암호화                         |
| **문제**  | OAuth 사용자는 비밀번호 없음 / 6자리 PIN은 취약 / 멀티 기기 지원 필요 |
| **해결**  | PIN + Server Key 하이브리드 → DB 유출만으로는 오프라인 공격 불가  |
| **한계**  | 서버 전체 침해 시 PIN 브루트포스 가능 (10⁶ 공간)               |
| **방어선** | PBKDF2 600k iterations, 침해 탐지, 향후 KMS/HSM      |
...
|---------------|---------------------|-----------------|------------------------------------|
| DB 유출         | Salt, Encrypted DEK | Server Key, PIN | 오프라인 공격 불가 (Server Key 부재)         |
| 서버 침해         | Server Key          | Salt, DEK, PIN  | 복호화 불가 (암호문 부재)                    |
| DB + 서버 동시 침해 | 모든 서버 데이터           | PIN             | PIN 오프라인 브루트포스 가능 (10⁶ 공간, KDF 비용) |

**DB 유출만으로는 오프라인 브루트포스가 불가능합니다.** 공격자가 DB를 통째로 가져가도 Server Key 없이는 로컬에서 PIN을 시도해볼 수 없습니다.

단, **서버(애플리케이션/Master Key)까지 침해**되어 Server Key 평문을 얻는 경우, PIN 공간(10⁶)에 대한 오프라인 브루트포스가 가능해집니다. 이 경우의 방어선은 Rate Limit이 아니라
**KDF 비용(PBKDF2 600k iterations)**, **침해 탐지/격리**, **Master Key 보호(향후 KMS/HSM)**입니다.
...
이는 의도된 제약입니다. 서버가 평문을 알 수 없는 구조를 유지하려면, 서버도 데이터를 복구할 수 없어야 합니다.
...
| 포기한 것     | 얻은 것                     |
|-----------|--------------------------|
| 서버 측 검색   | DB 유출만으로는 평문 복원 불가       |
| 관리자 복구 기능 | 서버 단독으로 평문 접근 불가         |
| 일부 편의 기능  | 서버 침해 시에도 추가 방어선(KDF) 존재 |
...
2. **DB 유출 방어**: Server Key 결합으로 DB 단독 유출 시 오프라인 공격 차단
3. **평문 비저장**: 서버는 암호문만 저장하며, 복호화 키는 사용자 입력(PIN/Recovery Key)에 의존

서버 DB가 유출되어도 평문 복원이 어렵고, 서버 단독으로는 사용자 데이터에 접근할 수 없습니다. 단, 서버(Master Key 포함) 전체가 침해되는 경우 PIN 엔트로피 한계가 존재하며, 이는 KDF 비용과 운영
보안(향후 KMS/HSM)으로 완화합니다.
